...
class Module01Sol2020Apr extends KoanSuite with Matchers {
  ...
  class CalorieTracker {
    private[this] var dM: Int = _
    private[this] var cD: Int = _

    def dailyMax: Int = dM
    def currentDaily: Int =  cD

    def dailyMax_=(valueToSet: Int): Unit = {
      require(valueToSet > 0 && valueToSet < 5000, "daily Max must be greater than 0 and less than 5000")
      dM = valueToSet
    }
    def currentDaily_=(valueToSet: Int): Unit =  {
      require(valueToSet > -500 && valueToSet < dM + 500, "currentDaily must be greater than -500 and " +
        s"less than ${dM + 500}")
      cD = valueToSet
    }
  ...
  test ("should enforce limits on daily") {
    ...
    intercept[IllegalArgumentException] { tracker.dailyMax = 5001 }
  }
  ...
  test ("should enforce limits on setting current daily calories based on tracker max") {
    ...
    intercept[IllegalArgumentException] { tracker1.currentDaily += 1000 } //should be less than 2500(tracker1.dailyMax+500)
    tracker2.currentDaily += 1000 //should be less than 3500(tracker2.dailyMax+500)
  ...
  test ("should enforce limits on the eat and exercise methods too") {
    ...
    //should be less than tracker1.dailyMax+500, that is 2500 but currentDaily becomes 1500+1000
    tracker2.exercise(1000)
    //should be less than tracker2.dailyMax+500, that is 3500 but currentDaily becomes 1500+1000 which is safe
    ...
    intercept[IllegalArgumentException] { tracker1.eat(1000) } //currentDaily becomes -500
    tracker2.eat(1000) //currentDaily becomes 500
  ...
  object SlowDB {
    val foods: Map[String, List[String]] = Map(
  ...
  object DBCache {
    private val recipeCache: LoadingCache[String, Future[List[String]]] = CacheBuilder.newBuilder().build {
  ...
  test ("cache should not take a long time to retrieve recipes for the same food") {
    ...
    if ((System.currentTimeMillis - startTime) > 1500) fail("Took too long, " +
      "must be doing more than one DB lookup for eggs")
  }
}
