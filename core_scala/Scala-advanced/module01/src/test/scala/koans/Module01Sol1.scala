...
class Module01Sol1 extends KoanSuite with Matchers {
  ...
  class CalorieTracker {
    ...
    private[this] var _dialyMax: Int = 0
    private[this] var _currentDaily: Int = 0

    def dailyMax =  _dialyMax
    def dailyMax_=(input:Int) =  {
      require(input > 0 && input < 5000, "the dailyMax must be larger than 0 and smaller than 5000 calories")
      _dialyMax = input
    }
    def currentDaily =  _currentDaily
    def currentDaily_=(input:Int) =  {
      require(input > -500 && input < dailyMax+500, "the currentDaily may only be set to values in a range from -500 to dailyMax + 500")
      _currentDaily = input
    }
