...
class Module10XML extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  case class Book(title: String, author: String, year: Int, pages: Int) extends LibraryItem {
    def toXML: Node = <book/> // needs to be replaced with a real implementation
  }

  object Book {
    def fromXML(node: Node) =
      Book("", "", 0, 0)  // replace with real implementation
  }

  case class Journal(title: String, issue: Int, month: Int, year: Int) extends LibraryItem {
    def toXML: Node = <journal/> // needs to be replaced with a real implementation
  }

  object Journal {
    def fromXML(node: Node) =
      Journal("", 0, 0, 0) // replace with real implementation
  }

  case class DVD(title: String, director: String, year: Int, length: Int) extends LibraryItem {
    def toXML: Node = <dvd/> // needs to be replaced with a real implementation
  }

  object DVD {
    def fromXML(node: Node) =
      DVD("", "", 0, 0) // replace with real implementation
  }

  case class CD(title: String, artist: String, year: Int, length: Int) extends LibraryItem {
    def toXML: Node = <cd/> // needs to be replaced with a real implementation
  }

  object CD {
    def fromXML(node: Node) =
      CD("", "", 0, 0) // replace with real implementation
  }
  ...
  koan("Attribute access") {
    ...
    def totalPrices(node: Node): Double = 0.0    // replace with real implementation
  ...
  koan("Parsing the XML file") {
    // In the project directory is a file called LibraryItems.xml - take a look at it if you like.
    ...
    def readInLibraryItems(fileName: String): Seq[LibraryItem] = Nil   // fill in real implementation
